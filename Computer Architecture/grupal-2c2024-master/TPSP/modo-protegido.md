...
El modo real es el modo operativo inicial que tiene un procesador Intel x86, para mantener compatibilidad simula el comportamiento de los procesadores 8086, operando en 16 bits. Esto es para proporcionar el entorno de programación del procesador Intel 8086, con algunas extensiones, como la capacidad de cambiar al modo protegido o al modo de administración del sistema.
El modo protegido es el modo operativo nativo del procesador que proporciona un amplio conjunto de caracteristicas modernas de los procesadores x86, como segmentación avanzada, implementa niveles de privilegio (CPL 0 a 3) para proteger el sistema operativo, acceso a memoria extendido, soporte para multitarea, mecanismos de deguridad.
...
Se debe hacer un pasaje del modo real al modo protegido ya que la mayoria de los sistemas operativos y aplicaciones modernas estan diseñadas para trabajar en modo protegido, dadas las caracteristicas que permite, como el soporte de multitarea, la implementacion de memoria virtual y paginacion, y mecanismos de proteccion del sistema operativo.
El problema con el modo real es principalmente su limitacion de memoria, ya que solo se puede direccionar hasta un 1MB de memoria, no existen niveles de privilegios ni mecanismos de proteccion, tampoco es posible la paginacion y la multitarea o manejo avanzado de interrupciones, lo que lo vuelve ineficiente e incompatible con muchos sistemas operativos que requieren las capacidades del modo protegido para funcionar correctamente.
...
La Global Descriptor Table es una estructura de datos utilizada en modo protegido para definir los segmentos de memoria. Cada entrada en la GDT es un Segment Descriptor que especifica la base, limite, tipo y privilegios de un semento particular. Esto permite al sistema operativo definir y gestionar diferentes segmentos de memoria con distintos niveles de privilegio y propiedades, facilitando la proteccion y organizacion de la memoria.
Los Segment Descriptors siguen el siguiente formato:
Segment Limit de 20 bits, define el tamaño del segmento. Si el bit G de granularidad esta establecido, el limite se multiplica por 4 KB.
- Bits 0-15: Limite bajo.
- Bits 16-19: Limite alto.
Base Address de 32 bits, especifica la direccion base donde comienza el segmento en memoria.
- Bits 0-15: Base baja.
- Bits 16-23: Base media.
- Bits 24-31: Base alta.
Type de 4 bits, define el tipo de segmento, es decir, indican si es de codigo, datos, TSS, etc., y sus permisos específicos de lectura, escritura, ejecucion.
S es un Descriptor Type de 1 bit, indica si el descriptor es de tipo sistema con S=0, o de tipo codigo/datos con S=1.
DPL de 2 bits, es el descriptor de nivel de privilegios del segmento, siendo de 0, el maximo privilegio, hasta a 3, el privilegio minimo.
P de 1 bit, indica si el segmento esta presente en memoria. Si el segmento no esta disponible, P=0, se genera una excepcion si se intenta acceder al mismo.
AVL es un 1 bit disponible para uso del sistema operativo.
L indica si el segmento utiliza el long mode, es decir, solo es aplicable en arquitecturas de 64 bits.
D/B en 1 bit define el tamaño de las operaciones en el segmento. Para segmentos de código, D=1 indica 32 bits; para segmentos de datos, B=1 indica que se utilizan operandos de 32 bits por defecto.
G es 1 bit para indicar la granularidad o escala del limite del segmento. G=0 establece el limite en bytes, y G=1 el limite en bloques de 4 KB.
...
Para un segmento de codigo con ejecución y lectura, los 4 bits del Type se configurarian tal que:
A = 0 inicialmente y se lo actualiza a 1 cuando se accede al segmento.
R = 1 para permitir la lectura.
C = 0 para segmentos de codigo estandar, ya que define si el segmento es conforme.
E = 1 para permitir la ejecucion.
...
GDT_DESC Contiene la dirección de memoria de la tabla y el tamaño en bytes del arreglo de descriptores.
Gdt Es el arreglo de todos los descriptores, apuntados por GDT_DESC
...
LGDT Toma una dirección de memoria en la que se encuentran los datos que necesita para cargar la GDT, la dirección de la tabla y su tamaño en bytes, y guarda los datos en el registro GDTR.
La estructura que indica la dirección de la GDT y su tamaño es gdt_descriptor_t, y se inicializa en gdt.c
...
Si, para pasar a modo protegido, entre otras cosas, hay que setear el primer bit de CR0
...
Como vamos a saltar al codigo del kernel, deberíamos saltar al segmento de codigo con privilegios de nivel 0 asi que hay que usar su selector.
...
Considerando que los pixeles tienen 2 bytes de longitud, el tamaño del segmento para el buffer de video deberia ser 50 x 80 x 2 x byte = 5000 bytes.
...
Daria la sensación de que dibuja un rectángulo en a partir de las coordenadas indicadas con el carácter y atributos pasados en los argumentos.
Para acceder a la memoria de video simplemente recurre a la macro que tiene la dirección de inicio del buffer de video.
Para representar un carácter utiliza la estructura “ca” que tiene un campo para el carácter y otro para los atributos. Ocupa 2 bytes en memoria.
