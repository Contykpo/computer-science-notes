...
El campo offset: se refiere al desplazamiento que hay que realizar desde el inicio del segmento
para llegar al punto de entrada de la rutina de servicio de la interrupcion.

El campo segsel: contiene el selector del segmento que contiene el codigo de atencion para la rutina

El campo type: Indica el tipo de compuerta

El campo dpl: Indica el nivel de privilegio requerido para poder llamar a esta interrupcion a nivel software (es decir, cuando la interrupcion se hace mediante una instruccion INT, INT3 o INTO).

El campo present: Indica si la gate esta activa.

El campo Gate Size: Indica el tamaño de los elementos que se colocaran en el stack cuando se ejecute una interrupcion.

El campo offset toma como valor la direccion de memoria de los distintos handlers.
...
El selector de segmento apropiado seria el de codigo de nivel 0 (indice 1, GDT, DPL = 0)

Para usar un Gate Size de 32 bits solamente hay que colocar el bit 42 en 1 y el resto queda igual.
...
IDT_ENTRY0 pues son interrupciones de hardware y no queremos que el codigo de usuario pueda llamarlas.
...
El prologo seria pushad mientras que el epilogo esta compuesto por popad y iret. Se utiliza iret en lugar de ret porque al estar volviendo de una interrupcion es necesario restaurar eip y cs, cosa que no hace la instruccion ret.
...
Para que el caracter giratorio se imprima una vez por segundo, guardaria una variable en memoria con el numero actual de interrupciones y lo compararia con la frecuencia de las interrupciones, si es igual entonces imprimo el caracter y reinicio el contador, y si es distinto entonces solo incremento el contador sin imprimir el caracter.