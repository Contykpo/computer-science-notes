...
uint16_t sched_next_task(void) {
  ...
  int8_t i;
  // Comienza desde el indice de current_task + 1, ya que se quiere evitar ejecutar la misma tarea nuevamente si hay otras disponibles.
  // La condicion i % MAX_TASKS != current_task garantiza que el bucle revisite las tareas en un ciclo, 
  // permitiendo que el scheduler vuelva al principio de la lista una vez que llegue al final.
  for (i = (current_task + 1); (i % MAX_TASKS) != current_task; i++) {
    // Si esta tarea está disponible la ejecutamos
    // Verifica si la tarea en el indice actual, ajustado por MAX_TASKS para garantizar que este dentro de los limites, tiene un estado de TASK_RUNNABLE.
